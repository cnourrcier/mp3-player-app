...
const playSong = (id) => {
    ...
    audio.src = song.src; // tells the audio element where to find the audio data for selected song.
    audio.title = song.title; // tells audio element what to display as the title of the song.
...
// shuffling the songs in the playlist and performe necessary state management updates after the shuffling.
const shuffle = () => {
...
const highlightCurrentSong = () => {
    const playlistSongElements = document.querySelectorAll(".playlist-song"); // querySelectorAll returns an array.
...
const renderSongs = (array) => {
    const songsHTML = array.map((song) => {
        return `<li id="song-${song.id}" class="playlist-song">
                <button class="playlist-song-info" onclick="playSong(${song.id})"><span class="playlist-song-title">${song.title}</span><span class="playlist-song-artist">${song.artist}</span><span class="playlist-song-duration">${song.duration}</span></button>
...
audio.addEventListener("ended", () => {
    const currentSongIndex = getCurrentSongIndex();
    const nextSongExists = true ? currentSongIndex + 1 < userData?.songs.length : false;
    // const nextSongExists = userData?.songs[currentSongIndex + 1] !== undefined; // alternate 
    if (nextSongExists) {
        playNextSong();
    } else {
        userData.currentSong = null;
